...
final class HomeVM: ObservableObject {
    ...
    /// Independent scan — fetches UTXOs + transactions for all addresses.
    /// Only runs ONCE per app session (hasLoadedOnce flag).
    func refreshBalance() async {
        guard !hasLoadedOnce else {
            #if DEBUG
            print("[HomeVM] refreshBalance() skipped — already loaded this session")
            #endif
            return
        }
        ...
        // Load addresses: cached from WalletVM or derive 20+20
        ...
        #if DEBUG
        print("[HomeVM] Balance: \(totalBalance) sats, \(result.utxos.count) UTXOs, \(dedupedTxs.count) txs")
        #endif
    }
